<%@page import="com.briup.bean.Article"%>
...
<link rel="stylesheet" type="text/css" href="${APP_PATH }/static/css/detail.css">
 <link rel="stylesheet" href="${APP_PATH }/static/css/forum.css">
...
<script type="text/javascript">
	$(function(){
		...
		$('.content_box #dz').on("click",function(){
			...
			$.get("likeArticle?articleId=${showArticle.id}&likeState="+islike,
			function(data){
				console.log(data);
				if(data == '1'){
		...
		$('.content_box #sc').on("click",function(){
			...
			$.get("collectionArticle?articleId=${showArticle.id}&collectionState="+isCollection,
				function(data){
					if(data == '1'){
						console.log("收藏成功");
      ...
      <!-- 右侧内容 -->
        ...
        <div class="content_box">
          ...
          <div class="new_header">
                <div class="new_header">
                   <div>作者：<b>${showArticle.user.nickname }</b></div>&nbsp;&nbsp;&nbsp;&nbsp;
                  <div>发表时间：<b>${showArticle.releaseDate }</b></div>
                </div>

               <div class="new_header">
               	<c:choose>
						<c:when test="${showArticle.isLike != 1 }">
		                  <div id="dz">
						</c:when>
						<c:otherwise>
                  	...
                  	点赞<b id="likeNum">${showArticle.likeTimes }</b>
                  </div>&nbsp;&nbsp;&nbsp;&nbsp;
                 <input type="hidden" name="islike" value="${showArticle.isLike }">
                  
                  
             	<c:choose>
					<c:when test="${showArticle.isCollection != 1 }">
	                  <div id="sc">
					</c:when>
					<c:otherwise>
                  ...
                  <span class="glyphicon glyphicon-star"></span>
                   	收藏<b id="collNum">${showArticle.collectionTimes }</b>
                  </div>&nbsp;&nbsp;&nbsp;&nbsp;
                  
                  <input type="hidden" name="isCollection" value="${showArticle.isCollection }">
                  
                  <c:choose>
						<c:when test="${showArticle.isReport == 0 }">
          ...
          <!--  下侧内容展示 -->
           ...
           <div class="new_content">
                      ...
                      ${showArticle.content }
                 <!--视频  -->
				<c:if test="${showArticle.videoUrl != null }">
					<video src="${showArticle.videoUrl }" controls="controls"  id="v1">
					</video>
				</c:if>
          </div>
...
<script type="text/javascript" src="${APP_PATH }/static/js/jquery.flexText.js"></script>
<!--textarea高度自适应-->
<script type="text/javascript">
    $(function () {
        $('.content').flexText();
